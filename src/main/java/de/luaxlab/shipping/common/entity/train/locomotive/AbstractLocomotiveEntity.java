...
import de.luaxlab.shipping.common.block.rail.MultiShapeRail;
import de.luaxlab.shipping.common.block.rail.blockentity.LocomotiveDockTileEntity;
...
import de.luaxlab.shipping.common.entity.navigator.LocomotiveNavigator;
...
import de.luaxlab.shipping.common.item.LocoRouteItem;
...
public abstract class AbstractLocomotiveEntity extends AbstractTrainCarEntity implements LinkableEntityHead<AbstractTrainCarEntity>, ItemHandlerVanillaContainerWrapper, HeadVehicle, MultiPartEntity, LLAbstractMinecartExtensions {
    @Setter
    ...
    private ItemStackHandler createLocoRouteItemHandler() {
        return new ItemStackHandler(1) {
            ...
            @NotNull
            @Override
            public long insertSlot(int slot, @NotNull ItemVariant stack, long maxAmount, TransactionContext transaction) {
    ...
    public float getMaxSpeedOnRail() {
        return (float) (ModConfig.Server.TRAIN_MAX_SPEED.get() * 0.9);
    }
        ...
        if(speedRecomputeCooldown < 0 || speedLimit < 0 ) {
            var dist = RailHelper.getRail(getOnPos().above(), this.level).flatMap(pos ->
                            railHelper.traverse(pos,
                                    ...
                                    12))
                    .orElse(12);
            double minimum = ModConfig.Server.LOCO_BASE_SPEED.get() * 0.2;
            double modifier = dist / 12d;
            speedLimit = minimum + (ModConfig.Server.LOCO_BASE_SPEED.get() * 0.8 * modifier);
